    ...
    path("send" / "email" / "request") {
      ...
      post {
        entity(as[EmailService.EmailModel]) { email =>
          val futRes = emailService.ask((replyTo: ActorRef[Response]) =>
            EmailService.SendEmailCommand(email, replyTo)
          )

          onComplete(futRes) {
            case Success(result) =>
              result match {
                case resp: SendEmailResponse =>
                  complete(
                    EmailHttpResponse(
                      ResponseStatus.Success,
                      "Email Sent Successfully",
                      Some(ResponseStatus.Success.id),
                      Some(resp.toJson)
                    )
                  )
                case resp: ErrorResponse =>
                  complete(
                    EmailHttpResponse(
                      ...
                      "Failed",
                      Some(ResponseStatus.BadRequest.id),
                      Some(resp.toJson)
                    )
                  )
              }
